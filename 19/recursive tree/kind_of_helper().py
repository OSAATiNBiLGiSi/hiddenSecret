...
for s in range(21):
    ...
    print(table[s])
    print(f"Bu Tablo'da Toplam {len(table[s])} Sayfa Var. Bu Tablo'nun Tüm Sayfa'larında {un_repeated_letters} Harflerinin Toplam Tekrar Sayısı")
    countOfTable = count_letters_in_table_of(table[s], un_repeated_letters)
    print(f"= {countOfTable} = 19 x {countOfTable/19}")
    print("\n\n\n")